...
$sql1 = "SELECT * FROM `manager_std` WHERE id_std = $id_std AND IsUse = 1;";
...
?>
	...
	<body id="page-top">
		...
		<header class="static-top fixed-top" style="background-color: #E60000;margin-top:120px;border-radius:100px;">
			<center style="padding-top:0px;">
				<font id="countDownTime" color="white" size="2px">เหลือเวลาทำข้อสอบ</font>
			</center>
		</header>
		<!-- <form style="font-size:14px;margin-top: 90px;padding-bottom:62px" id="frmSend" action="manager_send_exam_web.php" method="GET"> -->
		<form style="font-size:14px;margin-top: 120px;padding-bottom:55px" id="frmSend" action="manager_send_exam_std.php" method="POST">
	...
	WHERE manager_series_exam.id = $id_subject_get";
			...
			while ($row = mysqli_fetch_array($result, MYSQLI_ASSOC)) {
				...
				$list_series_exam = explode(';', $row['list_series_exam']);
				if (strstr($row['list_series_exam'], ";")) {
				...
				if (strstr($type_series_exam, ";")) {
					...
					$score_series_exam = explode(';', $row['score_series_exam']);
					...
					$exam_count2  = count(explode(',', $score_series_exam[1]));;
					// $score_series_exam1 = $score_series_exam1*$exam_count1;
					// $score_series_exam2 = array_sum($score_series_exam2);
				} else {
					if ($type_series_exam == 1) {
						...
						$list_series_exam1 = explode(',', $row['list_series_exam']);
						$exam_count1 = count($list_series_exam1);
						$score_series_exam1 = $row['score_series_exam'] * $exam_count1;
						$exam_count1 = count($list_series_exam1);
					}
					if ($type_series_exam == 2) {
						$type_series_exam1 = null;
						$type_series_exam2 = $type_series_exam;
						$list_series_exam2 = explode(',', $row['list_series_exam']);
						$score_series_exam2 = explode(',', $row['score_series_exam']);
						$score_series_exam2 = array_sum($score_series_exam2);
			...
			?>
				...
				​<?php
					if ($type_series_exam1 == 1) {
						echo '<div style="font-size:17px;" class="card-header"><b><u>แบบปรนัย ' . $exam_count1 . ' ข้อ ' . $score_series_exam1 . ' คะแนน</b></u></div>';
						// print_r($list_series_exam1);
						$arr_list_series_exam = $list_series_exam1;
						shuffle($arr_list_series_exam);
						$list_exam = implode(",", $arr_list_series_exam);
						// $list_exam = implode(",",$arr_list_series_exam);
					?>
				<input hidden name="list_series_exam1[]" value="<?php echo $list_exam; ?>" />
				<?php
						...
						for ($i_arr = 0; $i_arr < count($arr_list_series_exam); $i_arr++) {
							...
							while ($row1 = mysqli_fetch_array($result1, MYSQLI_ASSOC)) {
								...
								$answer5_exam =  $row1['answer5_exam'];
								$answer5_img_exam =  $row1['answer5_img_exam'];
				...
				?>
						...
						<div style="font-size:17px;">
							<div class="card-header">
								<center><img class="mb-2" style="border-radius: 10px;" <?php if ($proposition_img_exam == null) {
																							echo "hidden";
																						} ?> src="upload/<?php echo $proposition_img_exam; ?>" height="200px"></center>
							...
							<div class="radio-toolbar">
								<div class="card-body">
									...
									foreach ($ans_type_subject as $ans_type_subjects) { ?>
										<input id="radio<?php echo $ans_type_subjects; ?><?php echo $i; ?>" required type="radio" name="Ans1[<?php echo $i; ?>]" value="<?php echo $ans_type_subjects; ?>">
										<label for="radio<?php echo $ans_type_subjects; ?><?php echo $i; ?>" class="container-fluid">
											<center><img style="border-radius: 10px;" src="upload/<?php
																									if ($num_ans == 1) {
																										echo $answer1_img_exam . '" ';
																										if ($answer1_img_exam == null) {
																											echo "hidden";
																										}
																									} else if ($num_ans == 2) {
																										echo $answer2_img_exam . '" ';
																										if ($answer2_img_exam == null) {
																											echo "hidden";
																										}
																									} else if ($num_ans == 3) {
																										echo $answer3_img_exam . '" ';
																										if ($answer3_img_exam == null) {
																											echo "hidden";
																										}
																									} else if ($num_ans == 4) {
																										echo $answer4_img_exam . '" ';
																										if ($answer4_img_exam == null) {
																											echo "hidden";
																										}
																									} else if ($num_ans == 5) {
																										echo $answer5_img_exam . '" ';
																										if ($answer5_img_exam == null) {
																											echo "hidden";
																										}
																									}
																									?> width=" 50px" height="50px"></center>
											<?php
											if ($num_ans == 1) {
												echo $ans_type_subjects . ") " . $answer1_exam;
											} else if ($num_ans == 2) {
												echo $ans_type_subjects . ") " . $answer2_exam;
											} else if ($num_ans == 3) {
												echo $ans_type_subjects . ") " . $answer3_exam;
											} else if ($num_ans == 4) {
												echo $ans_type_subjects . ") " . $answer4_exam;
											} else if ($num_ans == 5) {
												echo $ans_type_subjects . ") " . $answer5_exam;
											}
											?>
					...
					if ($type_series_exam2 == 2) {
						echo '<div style="font-size:17px;" class="card-header"><b><u>แบบอัตนัย ' . $exam_count2 . ' ข้อ ' . $score_series_exam2 . ' คะแนน</b></u></div>';
						$arr_list_series_exam = $list_series_exam2;
						// shuffle($arr_list_series_exam);
						// print_r($arr_list_series_exam);
						$list_exam2 = implode(",", $arr_list_series_exam);
						// $list_exam = implode(",",$arr_list_series_exam);
					?>
					<input hidden name="list_series_exam2[]" value="<?php echo $list_exam2; ?>" />
					<?php
						$num2 = 1;
						for ($i_arr = 0; $i_arr < count($arr_list_series_exam); $i_arr++) {
							$id = $arr_list_series_exam[$i_arr];
					...
					?>
							...
							<div style="font-size:17px;">
								<div class="card-header">
									...
									<?php
									echo "ข้อที่ " . $num2++; ?>). <?php echo $proposition_exam;
																	?>
								</div>

								<div class="radio-toolbar">
									<div class="card-body"" >
									<textarea class=" form-control" name="Ans2[]" rows="10" cols="80" required></textarea>
								...
								<div class="modal-dialog">
									<div class="modal-content">
										...
										<div class="modal-body text-center mb-5">
											...
											<h2 style="margin-top:30px">ต้องการส่งข้อสอบ ?</h2>
											<div class="btn-group mt-4">
												<button type="button" class="btn btn-secondary btn-lg mr-2 rounded-lg" data-dismiss="modal">ยกเลิก</button>
												<button type="button" onclick="SendExamSTD()" data-toggle="modal" data-target="#myModal" class="btn btn-success btn-lg mr-2 rounded-lg">ยืนยัน</button>
											</div>
										</div>
									</div>
								</div>
		...
		<script>
			function SendExamSTD() {
				var form = $("form#frmSend");
				var test = $("label").find(".error");
				console.log(test);
				form.validate({
					// errorClass: "invalid",
					ignoreTitle: true,

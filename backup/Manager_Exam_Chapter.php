        ...
        <div class="card mb-3">
          <div class="card-header">
			...
			<?php
						...
						?>
				วิชา : <?php echo $name_subject;?> <?php if($sum_chapter<=0){echo "<b>ยังไม่มีบทเรียน</b>";} else{echo "มีอยู่ <b>".$sum_chapter."</b> บทเรียน";} ?>
          ...
          <div class="card-body">
            ...
            <div class="table-responsive" >
				<?php
						$sql = "SELECT * FROM `manager_chapter` WHERE name_name_subject = $id_chapter ORDER BY `manager_chapter`.`num_chapter` ASC";
						$result = mysqli_query($conn, $sql);
						$num_chapter = 1;
						while ($row = mysqli_fetch_array($result,MYSQLI_ASSOC)) {
							$data_id =  $row['id'];
              ...
              <!-- <a id="edit_chapter<?php echo $num_chapter;?>" href="#collapse<?php echo $num_chapter;?>" role="button" data-toggle="modal" data-target="#myModal<?php echo $data_id;?>"><button type="button" class="btn btn-warning"><i style='font-size:24px' class='fas'>&#xf303;</i></button></a> -->
							...
							<a id="add_chapter<?php echo $num_chapter;?>" href="Manager_Exam_Add.php?id_chapter=<?php echo $data_id;?>"><i class="no-collapsable fas fa-table" > บทที่ <?php echo $edit_num_chapter." ".$data_name_chapter; ?></i></a>
          ...
          <div id="collapse<?php echo $num_chapter;?>" class="panel-collapse collapse">
            <div class="table-responsive">
              <table class="" id="dataTable" width="100%" cellspacing="0">
                ...
                <!-- </thead> -->
                  <tr>
                    <th width="10%">วัตถุประสงค์</th>
                    <th width="60%">
					<pre style="height:200px;background-color:white" class="form-control mt-3" id="show_txarea<?php echo $num_chapter;?>" readonly><?php echo $data_objective_chapter;?></pre>
					</th>
					<th  width="30%">
				      <button id="update_chapter<?php echo $num_chapter;?>" hidden type="submit" class="btn btn-primary">อัพเดท</button>
							<!-- <a id="add_chapter<?php echo $num_chapter;?>" href="Manager_Exam_Add.php?id_chapter=<?php echo $data_id;?>"><button type="button" class="btn btn-success ml-3">เพิ่มข้อสอบบทที่ <?php echo $num_chapter;?></button></a> -->
							<a id="edit_chapter<?php echo $num_chapter;?>" href="#collapse<?php echo $num_chapter;?>" role="button" data-toggle="modal" data-target="#myModal<?php echo $data_id;?>"><button type="button" class="btn btn-warning ml-3"><i style='font-size:24px' class='fas'>&#xf303;</i></button></a>
							<a id="delete_chapter<?php echo $num_chapter;?>" href="Manager_Exam_Chapter_Sql.php?name_subject_id=<?php echo $id_chapter;?>&id=<?php echo $edit_id_chapter;?>&delete_chapter" onclick="return buttonDelete<?php echo $num_chapter;?>();"><button type="button" class="btn btn-danger"><i style='font-size:20px;' class='fas'>&#xf2ed;</i></button></a>
					</th>
                  </tr>
						</table>

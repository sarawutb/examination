<?php
 ...
 // if(isset($_POST["branch"])){
	...
	//$degree_after =$_REQUEST["degree_after"];
 	...
 	$sql2 = "SELECT DISTINCT `degree_std`,`section_std` FROM `manager_std` WHERE `branch_id_std` = $branch AND IsUse = 1 ORDER BY `manager_std`.`degree_std` ASC,`manager_std`.`section_std` + 0 ASC";
	$true = 0;
  $id_num = 1;

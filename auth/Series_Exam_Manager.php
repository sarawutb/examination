  ...
  <div id="wrapper">
	...
	if(isset($_GET["id_series_exam"])){
		...
		$sql1 = "SELECT * FROM `manager_series_exam` WHERE id = $id_series_exam";
					...
					//	echo count($arr_list_series_exam);
	}
    ...
    <div id="content-wrapper">
      <div class="container-fluid">
        ...
        <ol class="breadcrumb">
          ...
          <li class="breadcrumb-item"> 
		    <a href="index.php">จัดการรายวิชา</a>
		  </li>
		  <?php 
...
<form action="Series_Exam_Manager_Sql.php" method="POST">	
		...
		<div class="col-sm-2">
			...
			<select name="year_std_series_exam" class="form-control" required="required">
				...
				<?php
					$sql2 = "SELECT DISTINCT `year_std` FROM `manager_std` WHERE IsUse = 1 ORDER BY `manager_std`.`year_std` ASC";
		...
		<div class="form-group">
				...
				<input style="width:100%" type="text" name="name_series_exam" class="form-control" value="<?php if($manager=="edit"){echo $name_series_exam;} ?>" required="required" autofocus="autofocus"></input>  
		</div>
		
		<div class="form-group">
				<label>คะแนนข้อละ</label>
				<input style="width:15%" type="text" name="name_score_exam" class="form-control" value="<?php if($manager=="edit"){echo $score_series_exam;} ?>" required="required" autofocus="autofocus" spellcheck="false" oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');"></input>  
		</div>
		
		<div class="form-group">
			<div class="form-inline">
			<label for="">เวลา/วัน/เดือน/ปี เริ่มสอบ   &nbsp; </label>
			<input style="width:30%" type="text" class="form-control" name="Time" id="reservationtime" value="<?php 
